...
public class Graf {
    ...
    void componenteConexe() {
        ...
        for(int i = 0; i < listaAdiacenta.size(); i++) {
            if(!vizitat[i]) {
                nodVizitat(i, vizitat);
                count++;
                System.out.println();
            }
        }
        System.out.println("Graful are " + count + " componente conexe.");
    }

    private void nodVizitat(int v, boolean[] vizitat) {
        vizitat[v] = true;
        System.out.print(v + " ");
        for (int i = 0; i<listaAdiacenta.get(v).size(); i++) { //pentru nodurile adiacente nodului v
            if(!vizitat[listaAdiacenta.get(v).get(i)]) {
                nodVizitat(listaAdiacenta.get(v).get(i), vizitat);
            }
        }
    }
}
