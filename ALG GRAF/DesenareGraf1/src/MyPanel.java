			...
			public void mouseReleased(MouseEvent e) {
				if (!isDragging) {
					...
					for(int i =0;i<listaNoduri.size();i++) {
						if(e.getX()>listaNoduri.get(i).getCoordX() && e.getX()<listaNoduri.get(i).getCoordX() + node_diam
								&& e.getY()>listaNoduri.get(i).getCoordY() && e.getY()<listaNoduri.get(i).getCoordY()+ node_diam) {
							inNode = true;
						}
					}
					if(inNode==true) {
